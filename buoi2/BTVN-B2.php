...
if (isset($_GET['xoa'])) {
    ...
    if (isset($_SESSION['ds_lich'][$idx])) {
        ...
        $_SESSION['ds_lich'] = array_values($_SESSION['ds_lich']);
    }
    header('Location: ' . $_SERVER['PHP_SELF']);
    exit;
}
...
<style>
    ...
    table { border-collapse: collapse; width: auto; margin-top: 15px; font-size: 14px; }
    th, td { border: 1px solid #ccc; padding: 4px 10px; text-align: left; }
    th { background: #f0f0f0; }
    form { max-width: 300px; }
    button { padding: 8px 16px; margin-top: 10px; }
    .loi { background: #fee2e2; color: #991b1b; border: 1px solid #fca5a5; padding: 8px 12px; border-radius: 4px; max-width: 300px; margin-bottom: 10px; }
</style>
</head>
<body>
...
<?php if ($thongBaoLoi): ?>
    <div class="loi">⚠ <?= htmlspecialchars($thongBaoLoi) ?></div>
<?php endif; ?>
...
<form method="POST">
    ...
    <input type="text" name="ten_mon" value="<?= $dangSua !== null ? htmlspecialchars($lichDangSua['ten_mon']) : '' ?>" required>
    ...
    <input type="number" name="tiet" min="1" max="12" value="<?= $dangSua !== null ? $lichDangSua['tiet'] : '' ?>" required>

    <label>Phòng học</label>
    <input type="text" name="phong" value="<?= $dangSua !== null ? htmlspecialchars($lichDangSua['phong']) : '' ?>" required>

    <button type="submit" name="luu"><?= $dangSua !== null ? "Cập nhật" : "Thêm" ?></button>
    <?php if ($dangSua !== null): ?>
        <a href="<?= $_SERVER['PHP_SELF'] ?>" style="margin-left:8px;">Hủy</a>
    <?php endif; ?>
</form>
